...
```{r dlGsodStations_2, results = "hide"}
gsod_moshi <- dlGsodStations(usaf = moshi$USAF,
                             start_year = 1990, end_year = 2000,
                             dsn = paste0(getwd(), "/data/moshi/"),
                             unzip = TRUE)
...
# Visualize adjusted dataset
ggplot(aes(x = as.Date(Datetime), y = toCelsius(TEMP, 1)), data = jul_moshi) + 
  geom_line(aes(y = val)) + 
  geom_point(aes(y = toCelsius(TEMP, 1)), data = jul_moshi[outlier_id, ], 
             color = "red", size = 4) + 
  labs(x = "Time (yr)", y = expression("Temperature (" ~ degree ~ C ~ ")")) + 
  theme_bw()
```
